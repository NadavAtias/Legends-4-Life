...
//------------------------------------------------------------------
bool Audio::getMode(const bool type)
{
...
void  Audio::setVolume(const int val)
{
	if (val > 0)
	{
		if (m_musicVolume != 100)       m_musicVolume += val;
		if (m_soundEffectVolume != 100) m_soundEffectVolume += val;
	}

	else
	{
		if (m_musicVolume != 0)      m_musicVolume += val;
		if (m_soundEffectVolume != 0) m_soundEffectVolume += val;
	}
